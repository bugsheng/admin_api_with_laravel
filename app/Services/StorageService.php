...
class StorageService extends BaseService implements StorageInterface
{
    ...
    public function storeAliOssFile(string $storage_path, UploadedFile $file, bool $is_public = false)
    {
        ...
        $data = array_merge([
            'url'       => $is_public ? Storage::disk($options['disk'])
                ->url($result['save_path'] . '/' . $result['save_name']) : Storage::disk($options['disk'])
                ->temporaryUrl($result['save_path'] . '/' . $result['save_name'], now()->addMinutes(60)),
            'is_public' => $is_public
        ], $result);
